 ...
 * 获取版本
 */
TJY.getTJYMARTVersion = function(url){
	$.post(url,{},function(data,textStatus){
	   ...
	   if(json){
		   if(json.version && json.version!='same'){
			   $('.TJYmart-version-tips').show();
			   $('#TJYmart_version').html(json.version);
			   $('#TJYmart_down').attr('href',json.downloadUrl);
		   }
		   if(json.accredit=='no'){
			   $('.TJYmart-accredit-tips').show();
		   }
...
$(function(){
	 ...
	 * @type {object}
	 */
	TJY.conf = window.conf;
	/* 基础对象检测 */
	TJY.conf || $.error("TJYMart基础配置没有正确加载！");
	if(TJY.conf.ROUTES)TJY.conf.ROUTES = eval("("+TJY.conf.ROUTES+")");
	/**
	 * 解析URL
	...
	TJY.initU = function(url,vars){
		if(typeof vars === "string"){
			vars = this.parse_str(vars);
		}
		var newUrl = TJY.conf.ROUTES[url];
		...
		tmpv = urlparams = null;
		if(false !== TJY.conf.SUFFIX){
			newUrl += "." + TJY.conf.SUFFIX;
		}
		...
		newUrl = TJY.conf.APP + "/"+newUrl;
		return newUrl;
	}
	
	TJY.U0 = function(url, vars){
		if(!url || url=='')return '';
		...
		path[1] || $.error("TJY.U(" + url + ")没有指定控制器");
		...
		//url = url.replace(new RegExp("%2F","gm"),"+");
		url = TJY.conf.APP + "/"+url;
		return url;
	}
	TJY.U = function(url,vars){
		if(TJY.conf.ROUTES && TJY.conf.ROUTES[url]){
		    return TJY.initU(url,vars);
		}else{
			return TJY.U0(url, vars);
		}
	}

	TJY.AU = function(url, vars){
        if(!url || url=='')return '';
        ...
        path[0] || $.error("TJY.AU(" + url + ")没有指定控制器");
        path[1] || $.error("TJY.AU(" + url + ")没有指定接口");
